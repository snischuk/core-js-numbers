...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const denominator1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const denominator2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(numerator / (denominator1 * denominator2));
}
...
function toNumber(value, def) {
  ...
  return !Number.isNaN(parsedValue) ? parsedValue : def;
}
...
function getFibonacciNumber(index) {
  ...
  return index === 0 ? firstNumber : secondNumber;
}
